...
// Toggle if user has seen intro animation
function useToggleHasSeenIntroAnimation() {
  const {
    context: { hasSeenIntroAnimation },
    setContext,
  } = useContext(SiteContext)

  async function toggleHasSeenIntroAnimation(state) {
    setContext((prevState) => {
      return { ...prevState, hasSeenIntroAnimation: state }
    })
  }
  return toggleHasSeenIntroAnimation
}
